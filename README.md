...
Each `wp wc delete <type>` command removes only the products, orders, customers, coupons, bookings, or terms that this plugin generated -- real store data is never touched.

> **Note:** generated objects are only identifiable going forward from this feature's release. Data generated by an earlier version of this plugin has no marker to detect it, so it cannot be targeted by these commands and must still be cleaned up manually.

```bash
# Preview how many generated products would be deleted, without deleting anything
wp wc delete products --dry-run

# Delete all generated products (prompts for confirmation)
wp wc delete products

# Delete all generated orders without a confirmation prompt
wp wc delete orders --yes

# Delete all generated bookings, along with any order created for them
wp wc delete bookings --yes

# Delete all generated terms for a taxonomy
wp wc delete terms product_tag --yes
```

| Option | Description |
|---|---|
| `<taxonomy>` | Required only for `wp wc delete terms`: `product_cat` or `product_tag` |
| `--yes` | Skip the confirmation prompt |
| `--dry-run` | Show how many items would be deleted, without deleting them |

Available types: `products`, `orders`, `customers`, `coupons`, `bookings`, `terms`.
...
Each generator also fires an action after deleting a previously-generated object:

- `smoothgenerator_product_deleted` -- after a generated product is deleted
- `smoothgenerator_order_deleted` -- after a generated order is deleted
- `smoothgenerator_booking_deleted` -- after a generated booking is deleted (requires WooCommerce Bookings)
- `smoothgenerator_customer_deleted` -- after a generated customer is deleted
- `smoothgenerator_coupon_deleted` -- after a generated coupon is deleted
- `smoothgenerator_term_deleted` -- after a generated term is deleted
